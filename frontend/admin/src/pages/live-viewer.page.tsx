...
export const LiveViewerPage: FC = () => {
  ...
  async function refreshStatus(): Promise<void> {
    const response = await fetch(apiUrl(`/api/webget?deviceID=${encodeURIComponent(deviceID)}`));
    ...
    <LayoutContent fixed className="h-[calc(100vh-4rem)]">
      <div className="grid h-full grid-cols-[minmax(0,1fr)_360px] bg-slate-950 text-slate-100 max-lg:grid-cols-1">
        <main className="relative min-h-[520px] bg-black">
          <video ref={videoRef} className="h-full w-full object-contain" autoPlay muted playsInline onLoadedMetadata={() => setMetadata((value) => (value ? { ...value } : null))} />
        ...
        <aside className="flex min-h-0 flex-col gap-4 border-l border-white/10 bg-slate-900 p-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">浏览器接收原始视频并叠加板卡 metadata 检测框。</p>
          </div>

          <label className="grid gap-1 text-sm">
            <span className="text-slate-400">信令地址</span>
            <input className="rounded-md border border-white/10 bg-slate-950 px-3 py-2" value={signalUrl} onChange={(event) => setSignalUrl(event.target.value)} />
          </label>
          <label className="grid gap-1 text-sm">
          ...
          <label className="grid gap-1 text-sm">
            ...
            <input className="rounded-md border border-white/10 bg-slate-950 px-3 py-2" value={iceServersInput} placeholder="stun:stun.l.google.com:19302" onChange={(event) => setIceServersInput(event.target.value)} />
          </label>

          <div className="grid grid-cols-3 gap-2">
            <button className="inline-flex items-center justify-center gap-2 rounded-md bg-cyan-500 px-3 py-2 text-sm font-medium text-slate-950 hover:bg-cyan-400" onClick={() => void connect()}>
              <PlugIcon className="h-4 w-4" />连接
            </button>
            <button className="inline-flex items-center justify-center gap-2 rounded-md bg-slate-800 px-3 py-2 text-sm hover:bg-slate-700" onClick={() => void disconnect()}>
              <UnplugIcon className="h-4 w-4" />断开
            </button>
            <button className="inline-flex items-center justify-center gap-2 rounded-md bg-slate-800 px-3 py-2 text-sm hover:bg-slate-700" onClick={() => void refreshStatus().catch((error) => setDeviceStatus(String(error)))}>
              <RefreshCwIcon className="h-4 w-4" />状态
            </button>
          </div>

          <div className="rounded-md border border-white/10 bg-slate-950 p-3 text-sm text-slate-300">
            <div>{status}</div>
            <div className="mt-2 text-slate-400">距离：{metadata?.distance_m == null ? "-" : `${metadata.distance_m.toFixed(3)} m`}</div>
            <div className="text-slate-400">检测：{metadata?.detected ? "是" : "否"}</div>
          </div>

          <pre className="max-h-48 overflow-auto rounded-md border border-white/10 bg-slate-950 p-3 text-xs text-slate-400">{JSON.stringify(metadata || {}, null, 2)}</pre>
          <pre className="max-h-36 overflow-auto rounded-md border border-white/10 bg-slate-950 p-3 text-xs text-slate-400">{deviceStatus}</pre>
